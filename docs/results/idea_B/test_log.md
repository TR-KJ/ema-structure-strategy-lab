...
minRangeTicks = 50 固定で maxRangeTicks を 100 / 150 / 200 / 300 / 500 と比較した。

結果として、maxRangeTicks を狭めるほどPFは悪化した。

最も広い maxRangeTicks 500 でPF 0.350となり、Range幅フィルターなしのv1 RR2.0基準 PF 0.352 に近い結果となった。

このため、現時点では「広すぎるレンジを除外する」方向では改善しないと判断する。

平均トレードもすべて -15 前後であり、maxRangeTicks の調整による期待値改善は確認できなかった。
---
